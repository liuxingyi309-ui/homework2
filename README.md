...
The `compliance dashboard.ipynb` notebook is under 'outputs' file. It visualizes the structured compliance results generated by `src/compliance_checker.py` (Module 3). It provides an interactive view of trade validation outcomes across CFTC and EMIR regimes, using the `outputs/result.json` compliance report as its data source.
